...
static void iLinkIntrCBHandlingThread(void *arg){
 ...
 while(1){
	...
	if((nNodes=iLinkGetNodeCount())<0) M_DEBUG("Critical error: Failure getting the number of nodes!\n"); /* Error. */
	...
	for(i=0; i<nNodes; i++){
		...
		else M_DEBUG("Error allocating a transaction.\n");
	}
