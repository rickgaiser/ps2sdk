...
int fat_getFileStartCluster(fat_driver* fatd, const char* fname, unsigned int* startCluster, fat_dir* fatDir) {
	...
	M_DEBUG("Entering fat_getFileStartCluster\n");
...
int fat_mount(struct block_device* bd)
{
	...
	for (i = 0; i < NUM_DRIVES && fatd == NULL; ++i)
	{
		if (g_fatd[i] == NULL)
		{
			M_DEBUG("usb fat: allocate fat_driver %d!\n", sizeof(fat_driver));
	...
	if (fatd == NULL)
	{
		M_PRINTF("usb fat: unable to allocate drive!\n");
		return -1;
	}

	if (fatd->bd != NULL)
	{
		M_PRINTF("usb fat: mount ERROR: alread mounted\n");
...
{
	unsigned int i;
	M_DEBUG("usb fat: forceUnmount devId %i \n", bd->devNr);
